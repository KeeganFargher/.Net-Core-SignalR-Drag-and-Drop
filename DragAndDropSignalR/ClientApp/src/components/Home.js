...
export function Home() {
	...
	useEffect(() => {
		if (connection && !connection.connectionStarted) {
			...
			connection.on("ReceivePositionMoved", (result) => setQuotes((q) => reorder(q, result.source.index, result.destination.index)));
	...
	async function addItem() {
		const id = (Math.max(...quotes.flatMap((x) => x.content)) + 1).toString();
